<!-- faf:start -->
<!-- faf: slash-tokens | TypeScript | cli | Token Optimization for Context Engineers. 4.8 KB WASM. Sub-millisecond. Zero dependencies. -->
<!-- faf: claim=project.faf | family=FAF -->

# CLAUDE.md — slash-tokens

## What This Is

Token Optimization for Context Engineers. 4.8 KB WASM. Sub-millisecond. Zero dependencies.

## Stack

- **Language:** TypeScript

## Context

- **Who:** Context Engineers making LLM API calls
- **What:** Token Optimization for Context Engineers. Pre-call checks in 4.8 KB WASM. Sub-millisecond. Zero dependencies.
- **Why:** You don't know what a call costs until the invoice arrives. Slash tells you before the call leaves your machine.
- **Where:** npm (slash-tokens), slashtokens.com, mcpaas.live/slash
- **When:** v1.6.3, August 2026
- **How:** npm install slash-tokens / bunx slash-tokens

---

*STATUS: BI-SYNC ACTIVE — 2026-08-25T12:53:55.978Z*
<!-- faf:end -->
