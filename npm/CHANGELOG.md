...
### Changed
- **License/Brand language rewritten.** Merged the prior separate `Brand` and `License` sections into one. Leads with "Code: MIT. Fork it, ship it, change it, show it, share it, sell it." — six verbs, because *show* and *share* are community drivers. Brand protection follows in plain language ("don't represent your app as Slash") instead of the prior "violators will get a takedown request" phrasing. Consistent across the ecosystem (slash-nextjs, RadioFAF).
